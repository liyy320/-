...
<html class="x-admin-sm">
    <head>
        ...
        <link rel="stylesheet" href="${ctxStatic}/x-admin/css/font.css">
        <link rel="stylesheet" href="${ctxStatic}/x-admin/css/xadmin.css">
        <link rel="stylesheet" href="${ctxStatic}/x-admin/css/theme5.css">
        ...
        <script type="text/javascript" src="${ctxStatic}/x-admin/js/xadmin.js"></script>
        <!-- 让IE8/9支持媒体查询，从而兼容栅格 -->
        <!--[if lt IE 9]>
        ...
        <script>
            // 是否开启刷新记忆tab功能 var is_remember = false;
        </script>
    </head>
    <body class="index">
        <!-- 顶部开始 -->
        <div class="container">
            <div class="logo"><a href="./index.html">${fns:getConfig('productName')} v2.0</a></div>
            <div class="left_open"><a><i title="展开左侧栏" class="iconfont">&#xe699;</i></a></div>
            <ul class="layui-nav right" lay-filter="">
                <li class="layui-nav-item">
                    <a href="javascript:;">${fns:getUser().loginName}</a>
                    <dl class="layui-nav-child">
                        <!-- 二级菜单 -->
                        <dd><a onclick="xadmin.open('个人信息','http://www.baidu.com')">个人信息</a></dd>
                        <dd><a onclick="xadmin.open('修改密码','http://www.baidu.com')">修改密码</a></dd>
...
function initMenu(data){
	...
	for(var i=0;i<topMenuData.length;i++){
		...
		menuHtml += "<li><a href='javascript:;'><i class='iconfont left-nav-li' lay-tips='" + topMenuData[i].name + "'>&#xe6b8;</i><cite>" + topMenuData[i].name + "</cite><i class='iconfont nav_right'>&#xe697;</i></a>";
		
		menuHtml += initChildMenu(topMenuData[i], list);
...
function initChildMenu(menu, list){
	
	var result = "";
	
	var childData = forEachMenu(menu.id, list);
	
	if(childData.length != 0){result += "<ul class='sub-menu'>";}
	
	for(var k=0;k<childData.length;k++){
		
		var child = initChildMenu(childData[k], list);
		
		result += "<li><a onclick='xadmin.add_tab('" + childData[k].name + "','welcome1.html')'>";
		
		if(child != ""){
			
			result += "<i class='iconfont'>&#xe6a7;</i>";
		}
		
		result += "<cite>" + childData[k].name + "</cite></a>";
	...
	if(childData.length != 0){result += "</ul>";}
